...
def write_scalars(writer, model_name, history, n_epoch, epoch):
    writer.add_scalars(model_name + ' non-binary/loss', {'train': history['loss_train'],\
                                                               'val': history['loss_val']}, epoch)
    for metric in history:
        if 'loss' not in metric and '@' not in metric and 'roc_auc' not in metric:
            writer.add_scalar(f'{model_name}_binary/{metric}', history[metric], epoch)
        else:
            writer.add_scalar(f'{model_name}_non-binary/{metric}', history[metric], epoch)
